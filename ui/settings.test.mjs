...
test('gateways contribute selectable credential forms and revoke actions and assignments use the form-inventory contract', async () => {
  ...
  assert.equal(section.items[0].fields[0].value, 'control-a');
  assert.deepEqual(section.items[0].actions.map(action => action.label), ['Save new secret', 'Revoke']);
  ...
  try {
    await handleSettingsAction('rotate-gateway:Hatchet', { 'rotate-controller:Hatchet': 'control-a', 'rotate-secret:Hatchet': 'test-only' });
